...
class GameStoreView(ListView):
    model = Game
    template_name = 'store/game_store.html'
...
class GameListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Создаем диапазон от 1 до 10 для выбора количества игроков
    ...
    def get_queryset(self):
        ...
        # Получаем параметр количества игроков из запроса GET
        players = self.request.GET.get('players')

        # Фильтруем по количеству игроков, если параметр указан
        if players:
...
class CategoryDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['games'] = self.object.games.all()  # Используем related_name из модели Game
        return context
...
class DesignerCreateView(CreateView):
    ...
    template_name = 'designers/designer_form.html'
    success_url = reverse_lazy('designer_list')
...
class DesignerDeleteView(DeleteView):
    model = Designer
    template_name = 'designers/designer_confirm_delete.html'
    success_url = reverse_lazy('designer_list')
...
class OrderItemListView(ListView):
    ...
    def get_queryset(self):
        return OrderItem.objects.all().select_related('order', 'game')
...
class StockCreateView(CreateView):
    ...
    def get_form(self, form_class=None):
        ...
        form.fields['game'].queryset = Game.objects.exclude(stock__isnull=False)
        return form
